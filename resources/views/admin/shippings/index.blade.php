                ...
                <div style="position: relative; display: flex; align-items: center; justify-content: space-between;">
                    <div style="margin-bottom: 26px;">
                        <form action="{{ url()->current() }}" style="display: flex;width: 500px;">
                            <input type="text" name="city" class="form-control" placeholder="Ingrese el nombre de la ciudad" value="{{ $city ?? '' }}">
                        ...
                        @foreach($records as $item)
                            <tr>
                                ...
                                <td class="btn-actions-index">
                                    <c-switch status="{{ $item->is_active }}" url="{{ route($name . '.active') }}" id="{{ $item->id }}"></c-switch>
                                </td>    
                                <td class="btn-actions-index">
