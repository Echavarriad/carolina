                    ...
                    <tbody id="sortable" data-url="{{ route($name . '.order') }}">
                        @foreach($records as $item)
                            <tr id="{{ $item->id }}">
                                ...
                                <td>
                                    <upload-images :img="{{ json_encode($item->image) }}" id="{{ json_encode($item->id) }}" with="1386" height="670" folder="slider" model='Slider' field='image'></upload-images>
                                </td>   
                                <td class="btn-actions-index">
                                    ...
                                    @method('DELETE')
                                    <button  type="submit" class="btn btn-danger btn-flat btn-delete" title="Eliminar" data-name ='' data-table = 'este registro'><i class="fa fa-trash"></i></button> </form>
                                </td>
                            </tr>
                        @endforeach                
                    </tbody> 
