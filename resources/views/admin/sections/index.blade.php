            ...
            <div class="box-body">
              @if(config('app.debug'))
                <div class="pull-right">
                  <a href=" {{ route('sections.create') }} " class="btn btn-primary"><i class="fa fa-plus"></i> Nuevo</a>
                </div>
              @endif
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>
                    @if($item->seo)
                     <a href="{{ route('sections.edit' , [$item->id]) }}" class="btn btn-primary btn-flat" title="Contenidos"><i class="fa fa-area-chart"></i></a>
                     @endif
                  </td>
                  <td>
                    <a href="{{ route('contents.show' , [$item->id]) }}" class="btn btn-primary btn-flat" title="Contenidos"><i class="fa fa-bars"></i></a>
                  </td>
                  <td class="btn-actions-index">
                    <input type="checkbox" name="{{ $item->id }}" class="input-switch" {{ $item->show ? 'checked':''}} data-size="mini"> 
                  </td>
                </tr>
                @endforeach
